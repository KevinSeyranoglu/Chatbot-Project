...
path = os.getcwd()+'\\data\\paired_comments\\Paired_comment2.csv'
...
if load_model:
    model.load_weights(os.getcwd()+'\\weights\\chatbot_weightsV2_1.h5')

if train:
    model.compile(optimizer=keras.optimizers.Adam(), loss='categorical_crossentropy', metrics=['acc'])
    model.fit([encoder_inp, decoder_inp], decoder_final_output, epochs=10, batch_size=24) 

    model.save_weights(os.getcwd()+'\\weights\\chatbot_weightsV2_1.h5')
...
print("##########################################")
print("#       Le chatbot brisé v1.0            #")
print("##########################################")


enc_model , dec_model = make_inference_models()
prepro1 = ""
while prepro1 != 'exit chatbot':
    
    prepro1 = input("you : ")
    prepro1=preprocess(prepro1)
    prepro = [prepro1]
    try:
        txt = []
        for x in prepro:
        ...
        decoded_translation=re.sub(" i'"," I'",decoded_translation)
        if censor: 
            print("chatbot : ", profanity.censor(decoded_translation) )    
